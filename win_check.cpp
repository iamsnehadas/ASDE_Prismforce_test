...
using namespace std;
/*If, say, the 3rd and 4th enemies have power equal to INT_MAX. So, after 3rd enemy is recharged again and attacks
Abhimanyu from behind (while he is batling the 4th enemy), the total power will exceed INT_MAX. So, long long int(ll) is used
for enemy powers. Similar scenario can happen for the 7th and 8th enemies.*/
bool check(int no_of_boons, int no_of_recharges, int initial_power, ll enemy_power[]) {
    ...
    for(int i = 0; i < 11; i++) {
        /*
        If the enemy is the 3rd or 7th enemy, then they are endured with power to regenerate themselves once with half of 
        their initial power and can attack Abhimanyu from behind if he is battling in iteratively next circle, that is, 
        the 4th or 8th enemy. So, when the 4th or 8th enemy is encountered, the power of the 3rd and 7th 
        enemy respectively is added to the power of the 4th and 8th enemy.
        */
        if(i == 3 || i == 7) {
    ...
    for(int i = 0; i < 11; i++) {
        ...
        if(current_power < enemy_power[i]) {
            ...
            /*Here the current power of Abhimanyu is compared to the power of the enemy in the next circle.
            If, the power of the enemy in the next circle is greater than that of Abhimanyu, he checks if he can recharge himself.
            Otherwise, he checks if he can skip the next circle by using boons.
            */
